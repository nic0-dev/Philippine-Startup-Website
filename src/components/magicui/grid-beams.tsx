...
interface GridBeamsProps extends HTMLAttributes<HTMLDivElement> {
  ...
  rayColor?: string;
  rayBlendMode?: "screen" | "overlay" | "soft-light" | "multiply";
}
...
const LightRay = React.memo<LightRayProps>(
  ({
    ...
    return (
      <motion.div
        className={cn(
          "absolute pointer-events-none -top-[5%] left-[var(--ray-left)] w-[var(--ray-width)] h-[var(--ray-height)] origin-top blur-[var(--ray-blur)] translate-x-[-50%] rotate-[var(--ray-rotation)]",
          blendMode === "overlay"
            ? "mix-blend-overlay"
        ...
        style={
          {
            ...
            background: `linear-gradient(to bottom, color-mix(in oklch, ${color} 85%, transparent), transparent)`,
